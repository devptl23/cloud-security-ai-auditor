...
![AI-Powered Cloud Security Auditor Architecture](./images/architecture.jpg)

**What This Shows:**
- **Left Panel (Development & Operations):** You author infrastructure code (main.tf) and auditing scripts (auditor.py) locally, then execute them via terminal
- **Middle Panel (AWS Cloud Environment):** Terraform provisions S3 buckets (secure and vulnerable) in us-east-1. Python + Boto3 scans bucket configurations. AWS Bedrock AI analyzes findings
- **Right Panel (AI Security Reporting):** Intelligent audit report generated with security findings and recommendations
- **Data Flow:** Local execution → Infrastructure provisioning → Security scanning → AI analysis → Professional report
